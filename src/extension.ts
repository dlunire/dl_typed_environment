...
const types: TypeName[] = ['string', 'integer', 'float', 'numeric', 'boolean', 'uuid', 'email'];

// Tipos que aceptan operaciones aritméticas
const NUMERIC_TYPES: Set<TypeName> = new Set(['integer', 'float', 'numeric']);

// Compatibilidad de asignación por referencia: destino -> orígenes aceptables
const ASSIGNABLE: Record<TypeName, Set<TypeName>> = {
...
const ARITHMETIC_EXPRESSION_PATTERN =
    /^-?(?:\d+(?:\.\d+)?|[A-Z]+(?:_[A-Z]+)*_*)(?:\s*[+\-*/]\s*-?(?:\d+(?:\.\d+)?|[A-Z]+(?:_[A-Z]+)*_*))+$/;

const COMPARISON_EXPRESSION_PATTERN =
    /^-?(?:\d+(?:\.\d+)?|[A-Z]+(?:_[A-Z]+)*_*)\s*(?:<|>)\s*-?(?:\d+(?:\.\d+)?|[A-Z]+(?:_[A-Z]+)*_*)$/;
...
// ==========================================
// ESTRUCTURAS DE DATOS DEL LINTER
// ==========================================
interface Symbol {
    name: string;
    type: TypeName;
...
interface ParsedLine {
    ...
    raw: string;
    varName: string;
    varRange: { start: number; end: number };
    declaredType: string;
    typeRange: { start: number; end: number };
    value: string;
    valueRange: { start: number; end: number };
    /** Errores estructurales detectados durante el parseo (fatales). */
 ...
 */
function extractJsDocAbove(document: vscode.TextDocument, declarationLine: number): string | undefined {
    const end = declarationLine - 1;
    if (end < 0) return undefined;

    const endLineText = document.lineAt(end).text.trim();
    if (!endLineText.endsWith('*/')) return undefined;

    let start = end;
    while (start >= 0) {
        const text = document.lineAt(start).text.trim();
        if (text.startsWith('/**')) break;
        if (!text.startsWith('*') && !text.endsWith('*/')) return undefined; // no era un bloque contiguo
        start--;
    }
 ...
 */
function normalizeJsDocBlocks(document: vscode.TextDocument): vscode.TextEdit[] {
    const edits: vscode.TextEdit[] = [];
    let start = -1;
    let indent = '';
...
export function activate(context: vscode.ExtensionContext): void {
    ...
    // 1. AUTOCOMPLETADO
    const providerParam: vscode.CompletionItemProvider = {
        provideCompletionItems(document, position) {
            const linePrefix = document.lineAt(position).text.substring(0, position.character);
            if (!VARIABLE_COLON_PATTERN.test(linePrefix)) return undefined;
    ...
    context.subscriptions.push(
        vscode.languages.registerCompletionItemProvider(LANGUAGE_ID, providerParam, ":")
    );
    ...
    context.subscriptions.push(
        vscode.languages.registerHoverProvider(LANGUAGE_ID, {
            provideHover(document, position) {
                ...
                if (!VAR_NAME_PATTERN.test(word)) return undefined;
                ...
                for (let i = 0; i < document.lineCount; i++) {
                    ...
                    if (doc) {
                        ...
                        markdown.appendMarkdown(description[declaredType as TypeName]);
                    }
    ...
    function updateDiagnostics(document: vscode.TextDocument): void {
        ...
        for (let i = 0; i < document.lineCount; i++) {
            ...
            if (!parsed) continue; // línea vacía o comentario puro

            parsedLines.push(parsed);

            if (parsed.structuralError) continue;

            // Redeclaración
            if (symbols.has(parsed.varName)) {
                const prev = symbols.get(parsed.varName)!;
                addError(
                    diagnostics, i, parsed.varRange.start, parsed.varRange.end,
                    `La variable '${parsed.varName}' ya fue declarada en la línea ${prev.line + 1}.`
                );
            } else if (types.includes(parsed.declaredType as TypeName)) {
                // Solo registramos si el tipo es válido
                symbols.set(parsed.varName, {
                    name: parsed.varName,
                    type: parsed.declaredType as TypeName,
                    line: i
                });
            }
        }
    ...
    vscode.workspace.onDidChangeTextDocument(
        e => updateDiagnostics(e.document), null, context.subscriptions
    );
    vscode.window.onDidChangeActiveTextEditor(
        editor => { if (editor) updateDiagnostics(editor.document); },
        null, context.subscriptions
    );
    vscode.workspace.onDidOpenTextDocument(
        doc => updateDiagnostics(doc), null, context.subscriptions
    );
...
// ==========================================
enum FSMState {
    START, READ_VAR, EXPECT_COLON, PRE_TYPE, READ_TYPE,
    EXPECT_EQUALS, PRE_VALUE, READ_VALUE, DONE
}
 ...
 */
function parseLine(raw: string, line: number, diagnostics: vscode.Diagnostic[]): ParsedLine | null {
    const trimmed = raw.trim();
    ...
    while (i < raw.length && state !== FSMState.DONE) {
        ...
        switch (state) {
            case FSMState.START:
                if (ws) break;
                if (isComment(i)) { state = FSMState.DONE; break; }
                if (/[A-Z]/.test(c)) {
                    result.varRange.start = i;
                    result.varName += c;
                    state = FSMState.READ_VAR;
                } else {
                    addError(diagnostics, line, i, i + 1,
                        `Sintaxis inválida. Una declaración debe iniciar con el nombre de variable en MAYÚSCULAS, pero se encontró '${c}'.`);
            ...
            case FSMState.READ_VAR:
                if (/[A-Z_]/.test(c)) {
                    ...
                    addError(diagnostics, line, result.varRange.start, i + 1,
                        `Carácter '${c}' no permitido en el nombre de variable. Solo se admiten MAYÚSCULAS y guiones bajos.`);
                    result.structuralError = true;
                    return result;
                }
                break;
            ...
            case FSMState.EXPECT_COLON:
                if (ws) break;
                if (c === ':') { state = FSMState.PRE_TYPE; }
                else {
                    addError(diagnostics, line, i, i + 1,
                        `Se esperaba ':' después de la variable '${result.varName}', pero se encontró '${c}'.`);
                    result.structuralError = true;
                    return result;
                }
                break;
            ...
            case FSMState.PRE_TYPE:
                ...
                if (/[a-z]/.test(c)) {
                    ...
                    addError(diagnostics, line, i, i + 1,
                        `Se esperaba un tipo primitivo después de ':', pero se encontró '${c}'.`);
                    result.structuralError = true;
                    return result;
                }
                break;
            ...
            case FSMState.READ_TYPE:
                if (/[a-z]/.test(c)) {
                    ...
                    addError(diagnostics, line, result.typeRange.start, i + 1,
                        `Sintaxis inválida en la declaración del tipo. Se encontró '${c}'.`);
                    result.structuralError = true;
                    return result;
                }
                break;
            ...
            case FSMState.EXPECT_EQUALS:
                if (ws) break;
                if (c === '=') { state = FSMState.PRE_VALUE; }
                else {
                    addError(diagnostics, line, i, i + 1,
                        `Se esperaba '=' después del tipo '${result.declaredType}', pero se encontró '${c}'.`);
                    result.structuralError = true;
                    return result;
                }
                break;
    ...
    if (result.varRange.end === 0 && result.varName) result.varRange.end = result.varRange.start + result.varName.length;
    if (result.typeRange.end === 0 && result.declaredType) result.typeRange.end = result.typeRange.start + result.declaredType.length;
    ...
    if (state !== FSMState.DONE) {
        if (state === FSMState.READ_VAR || state === FSMState.EXPECT_COLON) {
            addError(diagnostics, line, result.varRange.start, raw.length + 1,
                `Declaración incompleta: falta ':' y el tipo para la variable '${result.varName}'.`);
            result.structuralError = true;
        } else if (state === FSMState.PRE_TYPE) {
            addError(diagnostics, line, 0, raw.length + 1,
                `Declaración incompleta: falta el tipo después de ':'.`);
            result.structuralError = true;
        } else if (state === FSMState.READ_TYPE || state === FSMState.EXPECT_EQUALS) {
            addError(diagnostics, line, 0, raw.length + 1,
                `Asignación incompleta: falta '=' y el valor para '${result.varName}'.`);
            result.structuralError = true;
        } else if (state === FSMState.PRE_VALUE ||
            (state === FSMState.READ_VALUE && result.value.length === 0)) {
            addError(diagnostics, line, 0, raw.length + 1,
                `Asignación incompleta: se esperaba un valor al final de la línea.`);
            result.structuralError = true;
        }
    }

    // Tipo desconocido (estructuralmente correcto pero tipo inválido)
    if (!result.structuralError && !types.includes(result.declaredType as TypeName)) {
        addError(diagnostics, line, result.typeRange.start, result.typeRange.end,
            `Tipo desconocido: '${result.declaredType}'. Tipos válidos: ${types.join(', ')}.`);
...
function validateSemantics(
    ...
    const targetType = p.declaredType as TypeName;
    const value = p.value;
    ...
    if (isReference) {
        ...
        if (!ref) {
            addError(diagnostics, p.line, start, end,
                `Referencia inválida: la variable '${value}' no ha sido declarada.`);
            return;
        }
        if (!ASSIGNABLE[targetType].has(ref.type)) {
            addError(diagnostics, p.line, start, end,
                `Error de tipos: no se puede asignar '${value}' (tipo '${ref.type}') a una variable de tipo '${targetType}'.`);
        }
        return;
    }
    ...
    if (isComparisonExpression) {
        if (targetType !== 'boolean') {
            addError(
                ...
                `Error de tipos: las expresiones de comparación producen un valor 'boolean', no '${targetType}'.`
            );
            return;
        }
    ...
    if (isArithmeticExpression) {
        if (!NUMERIC_TYPES.has(targetType)) {
            addError(
                ...
                `Error de tipos: las expresiones aritméticas solo son válidas para tipos numéricos, no para '${targetType}'.`
            );
            return;
        }
...
function validateExpression(
    ...
    for (const op of operands) {
        if (VAR_NAME_PATTERN.test(op)) {
            ...
            if (!ref) {
                addError(diagnostics, p.line, p.valueRange.start, p.valueRange.end,
                    `Referencia inválida en la expresión: la variable '${op}' no ha sido declarada.`);
                return;
            }
            if (!NUMERIC_TYPES.has(ref.type)) {
                addError(diagnostics, p.line, p.valueRange.start, p.valueRange.end,
                    `Error de tipos: '${op}' es de tipo '${ref.type}' y no puede usarse en una expresión aritmética.`);
                return;
            }
        } else if (!/^-?\d+(\.\d+)?$/.test(op)) {
            addError(diagnostics, p.line, p.valueRange.start, p.valueRange.end,
                `Operando inválido en la expresión aritmética: '${op}'.`);
            return;
        }
    }
}
        ...
        if (VAR_NAME_PATTERN.test(op)) {
            ...
            if (!ref) {
                addError(
                    ...
                    p.valueRange.start,
                    p.valueRange.end,
                    `Referencia inválida en la comparación: la variable '${op}' no ha sido declarada.`
                );
                return;
            }
            ...
            if (!NUMERIC_TYPES.has(ref.type)) {
                addError(
                    ...
                    p.valueRange.start,
                    p.valueRange.end,
                    `Error de tipos: '${op}' es de tipo '${ref.type}' y no puede utilizarse en una comparación numérica.`
                );
                return;
            }
        } else if (!/^-?\d+(\.\d+)?$/.test(op)) {
            addError(
                ...
                p.valueRange.start,
                p.valueRange.end,
                `Operando inválido en la comparación: '${op}'.`
            );
            return;
        }
    }
 ...
 */
function registerJsdocAutoSpace(context: vscode.ExtensionContext): void {
    const disposable = vscode.workspace.onDidChangeTextDocument(event => {
        if (event.document.languageId !== LANGUAGE_ID) return;
        if (event.contentChanges.length === 0) return;

        const change = event.contentChanges.find(
        ...
        if (beforeCursor.endsWith('/** ')) return; // evita duplicar el espacio

        editor.edit(
            editBuilder => editBuilder.insert(position, ' '),
            { undoStopBefore: false, undoStopAfter: false } // un solo "Ctrl+Z" deshace todo
        ).then(success => {
            if (!success) return;
...
function addError(
    diagnostics: vscode.Diagnostic[],
    line: number, startCol: number, endCol: number, message: string
): void {
